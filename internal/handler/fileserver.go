...
func (fs *FileServer) ServeFileStream(req *protocol.Request, conn *tcp.TCPConn) error {
	...
	// Prevent directory traversal outside root
	if strings.Contains(cleanPath, "..") {
		return fs.sendError(conn, 403, "Forbidden", req.Version)
	}
	...
	if err != nil {
		if os.IsNotExist(err) {
			return fs.sendError(conn, 404, "Not Found", req.Version)
		}
		return fs.sendError(conn, 500, "Internal Server Error", req.Version)
	}
	...
	if fileInfo.IsDir() {
		...
		if newInfo, err := os.Stat(indexPath); err == nil {
			...
			return fs.sendError(conn, 403, "Directory listing disabled", req.Version)
		}
	}
	...
	if fileSize <= MaxInMemorySize {
		...
		return fs.serveSmallFile(conn, filePath, fileSize, modTime, req.Version, req)
	} else {
		// Large file: Use streaming with Range support (memory-efficient)
		return fs.serveLargeFile(req, conn, filePath, fileSize, modTime, req.Version)
	}
}
...
func (fs *FileServer) serveSmallFile(conn *tcp.TCPConn, filePath string, fileSize int64, modTime time.Time, version protocol.HTTPVersion, req *protocol.Request) error {
	content, err := os.ReadFile(filePath)
	if err != nil {
...
func (fs *FileServer) serveLargeFile(req *protocol.Request, conn *tcp.TCPConn, filePath string, fileSize int64, modTime time.Time, version protocol.HTTPVersion) error {
	...
	if rangeHeader == "" {
		...
		return fs.sendFullFile(file, filePath, fileSize, modTime, version, conn)
	}
	...
	return fs.sendRangeFile(file, start, end, filePath, fileSize, modTime, version, conn)
}
...
func (fs *FileServer) sendFullFile(file *os.File, filePath string, fileSize int64, modTime time.Time, version protocol.HTTPVersion, conn *tcp.TCPConn) error {
...
func (fs *FileServer) sendRangeFile(file *os.File, start, end int64, filePath string, fileSize int64, modTime time.Time, version protocol.HTTPVersion, conn *tcp.TCPConn) error {
...
func (fs *FileServer) sendError(conn *tcp.TCPConn, code int, status string, version protocol.HTTPVersion) error {
...
func HandleStaticFileStream(rootDir string) func(*protocol.Request, *tcp.TCPConn) error {
	fs := NewFileServer(rootDir)
	return func(req *protocol.Request, conn *tcp.TCPConn) error {
		return fs.ServeFileStream(req, conn)
	}
}
